...
class Trainer:
    def __init__(self, exp: Exp, args):
        ...
        # early stopping (lido do Exp; patience=0 desativa). Conta avaliacoes sem melhora
        # do AP50:95 de validacao; com eval_interval=1 isso equivale a epocas.
        self.early_stop_patience = getattr(exp, "early_stop_patience", 0)
        self.early_stop_min_delta = getattr(exp, "early_stop_min_delta", 0.0)
    ...
    def train_in_epoch(self):
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            if self.early_stop:
                logger.info(
                    "Early stopping: {} avaliacoes sem melhora. Melhor AP50:95 = {:.4f}".format(
                        self.early_stop_patience, self.best_ap
                    )
                )
                break
    ...
    def evaluate_and_save_model(self):
        ...
        update_best_ckpt = ap50_95 > self.best_ap + self.early_stop_min_delta
        if update_best_ckpt:
            self.no_improve_count = 0
        else:
            ...
            if self.early_stop_patience > 0:
                logger.info(
                    "Sem melhora ha {}/{} avaliacoes (melhor AP50:95 = {:.4f})".format(
                        self.no_improve_count, self.early_stop_patience, self.best_ap
                    )
                )
